...
function Announcement({ title, time, isOn }) {
    ...
    return (
        <div className='bg-[#FAFAFA] rounded-md flex justify-between items-center p-[10px] gap-[15px]'>
            <div className={`flex flex-col justify-between gap-[15px] {${isOn ? "w-[60%]" : "w-[100%]"}}`}>
                <div className='text-[16px] font-medium'>{title}</div>
                <div className=''>{time}</div>
            </div>
            <div className='flex justify-center items-center gap-[20px]'>
                <a className={isOn ? 'flex' : 'hidden'} href="#" onClick={handlePin}>
